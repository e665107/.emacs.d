...
(add-hook 'snippet-mode-hook 'hs-minor-mode)
...
(global-set-key (kbd"C-o") 'hs-hide-level)

(defun hs-all-toggle-hiding ()
...
;; (defun hs-ifdef-block-toggle ()
;;   "Toggle hiding/showing of a block.
;; See `hs-hide-ifdef-block' and `hs-show-ifdef-block'."
;;   (interactive)
;;   (hs-life-goes-on
;;    (if (hs-already-hidden-p)
;;        (show-ifdef-block)
...
(global-set-key (kbd"C-'") 'hs-all-toggle-hiding)
